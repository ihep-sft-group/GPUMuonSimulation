...
#cmt_local_tagfile_install_more_includes = $(MFSSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(MFSSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)MFSSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)MFSSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "MFSSvc" = ""; then \
	    if test -d ../MFSSvc; then \
	      echo "Installing files from more ../MFSSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/MFSSvc; then mkdir -p $(install_include_dir)/MFSSvc; fi;\
	        $(install_command) ../MFSSvc $(install_include_dir);\
	    else \
	      echo "No more MFSSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/MFSSvc"; \
	  $(uninstall_command) "$(install_include_dir)/MFSSvc/*" ; \
	  if test -f $(install_include_dir)/MFSSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/MFSSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/MFSSvc.cmtref ; \
	  fi; \
	fi
